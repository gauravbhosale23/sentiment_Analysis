...
random_tweet = st.sidebar.radio('Sentiment', ('positive', 'neutral', 'negative'), key='random_tweet')
st.sidebar.markdown(data.query("airline_sentiment == @random_tweet")[["text"]].sample(n=1).iat[0, 0])

# Number of tweets by sentiment
st.sidebar.markdown("### Number of tweets by sentiment")
select = st.sidebar.selectbox('Visualization type', ['Bar plot', 'Pie chart'], key='sentiment_select')
...
if not st.sidebar.checkbox("Close", True, key='location_checkbox'):
...
each_airline = st.sidebar.selectbox('Visualization type', ['Bar plot', 'Pie chart'], key='airline_select')
...
if not st.sidebar.checkbox("Close", True, key='airline_checkbox'):
...
# Plot sentiment breakdown by airline
@st.cache(persist=True)
...
if len(choice) > 0:
    ...
    breakdown_type = st.sidebar.selectbox('Visualization type', ['Pie chart', 'Bar plot'], key='breakdown_type_select')
...
# Histogram for breakdown by sentiment
st.sidebar.subheader("Breakdown airline by sentiment")
choice = st.sidebar.multiselect('Pick airlines', ('US Airways','United','American','Southwest','Delta','Virgin America'), key='histogram_airline_select')
if len(choice) > 0:
...
word_sentiment = st.sidebar.radio('Display word cloud for what sentiment?', ('positive', 'neutral', 'negative'), key='wordcloud_sentiment_radio')
# st.set_option('deprecation.showPyplotGlobalUse', False)
if not st.sidebar.checkbox("Close", True, key='wordcloud_checkbox'):
